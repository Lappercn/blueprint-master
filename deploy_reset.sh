...
if [ -d "$BLUEPRINT_DIR" ]; then
    ...
    if [ -d "$BLUEPRINT_DIR/frontend" ]; then
        ...
        # 使用 VITE_BASE_URL 指定子路径：
        # - VITE_API_BASE_URL：蓝图大师后端统一走 api.tongzhilian.cn
        # - VITE_SSE_API_BASE_URL：保持一致（流式接口同样走 api.tongzhilian.cn）
        docker run --rm \
            -v "$BLUEPRINT_DIR/frontend":/app \
            -w /app \
            -e VITE_API_BASE_URL="https://api.tongzhilian.cn/api/v1" \
            -e VITE_SSE_API_BASE_URL="https://api.tongzhilian.cn/api/v1" \
            node:20-alpine sh -c "
                npm install && 
    ...
    if [ -d "$BLUEPRINT_DIR/backend" ]; then
        ...
        docker run -d \
            ...
            python:3.10-slim sh -c "
                pip install --upgrade pip --default-timeout=1000 -i https://pypi.tuna.tsinghua.edu.cn/simple &&
                pip install -r requirements.txt --default-timeout=1000 -i https://pypi.tuna.tsinghua.edu.cn/simple && 
                gunicorn -w 2 -k gevent --worker-connections 1000 -b 0.0.0.0:5000 --timeout 600 wsgi:app
            "
        echo "    蓝图大师后端已启动"
    fi
else
